...
pub trait WorkListAlgo {
        ...
        Self::OutFlowType: Clone + Eq,
    {
        ...
        let mut in_states: HashMap<NodePtr, Self::InFlowType> = HashMap::new();
        ...
        while let Some(ref next_to_do) = worklist.pop_front() {
            ...
            let in_flow = if !pred_out_flow.is_empty() {
                let merged_in_flow = Self::merge(pred_out_flow);
                in_states.insert(next_ptr, merged_in_flow.clone());
                merged_in_flow
            } else {
                in_states
                    .entry(next_ptr)
                    .or_insert(self.init_in_flow_state(next_to_do))
                    .clone()
            };
